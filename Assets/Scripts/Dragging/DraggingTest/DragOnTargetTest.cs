...
public class DragOnTargetTest : DraggingActionsTest
{
    ...
    public override void OnDraggingInUpdate()
    {
        ...
        float distanceToTarget = (direction* 180f).magnitude;
        if (notNormalized.magnitude > distanceToTarget)
        {
            // draw a line between the creature and the target
            lr.SetPositions(new Vector3[]{ transform.parent.position, transform.position - direction*180f });
            lr.enabled = true;

            // position the end of the arrow between near the target.
            triangleSR.enabled = true;
            triangleSR.transform.position = transform.position - 130f*direction;
